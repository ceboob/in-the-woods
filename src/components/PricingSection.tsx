      ...
      >
        ...
        <div className="grid md:grid-cols-2 gap-6 max-w-3xl mx-auto mb-10">
          {packages.map((pkg, i) => (
            <div
              key={i}
              className={`${pkg.featured ? 'border-2 border-forest' : 'border border-border'} bg-card p-8 text-center space-y-4 relative`}
            >
              {pkg.featured && (
        ...
        <div className="text-center space-y-3">
          <p className="text-xs text-muted-foreground">
            Cena zależy od terminu, liczby gości i sezonu. Wyślij zapytanie, aby otrzymać dokładną
            ofertę.
          </p>
